...
#define MAX_MODE1_BOUNCING 50
#define MAX_MODE2_BOUNCING 90
...
void* volume_execute_thread()
{
    while(isTerminate == 0)
    {
        ...
        if(volumeContinue >= MAX_BOUNCING && prevVolumeDir == 2)
        {
            ...
            snprintf(volumeBuffer, MAX_BUFFER_SIZE, "volume_set,%d", AudioMixerControl_getVolume());
...
void* tempo_execute_thread()
{
    while(isTerminate == 0)
    {
        ...
        if(tempoContinue >= MAX_BOUNCING && prevTempoDir == 2)
        {
            ...
            //Send to Server -> update on frontend
            memset(tempoBuffer, 0, sizeof(tempoBuffer));
            snprintf(tempoBuffer, MAX_BUFFER_SIZE, "tempo_set,%d", AudioMixerControl_getTempo());
